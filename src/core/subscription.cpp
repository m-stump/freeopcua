  ...
  for (const NotificationData & data : result.NotificationMessage.NotificationData)
    {
      if (data.Header.TypeId == ExpandedObjectId::DataChangeNotification)
        ...
        {
          LOG_WARN(Logger, "subscription          | unknown notficiation type received: {}", fmt::streamed(data.Header.TypeId).value());
        }
    }
  ...
  for (const MonitoredItems & item : data.DataChange.Notification)
    {
      ...
      if (mapit == AttributeValueMap.end())
        {
          LOG_WARN(Logger, "subscription          | got PublishResult for an unknown monitoreditem id: {}", fmt::streamed(item.ClientHandle).value());
        }
      ...
      else
        {
          ...
          LOG_DEBUG(Logger, "subscription          | calling DataChange user callback: {} and node: {}", fmt::streamed(item.ClientHandle).value(), fmt::streamed(mapit->second.TargetNode).value());
  ...
  for (EventFieldList ef : data.Events.Events)
    {
      ...
      if (mapit == AttributeValueMap.end())
        {
          LOG_WARN(Logger, "subscription          | got PublishResult for an unknown MonitoredItem id: {}", fmt::streamed(ef.ClientHandle).value());
        }
  ...
  for (Node & child : eventtype.GetProperties())
    {
      ...
      LOG_DEBUG(Logger, "  property: {}", fmt::streamed(propertyName).value());
